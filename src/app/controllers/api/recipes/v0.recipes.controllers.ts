...
export const v0GetRecipesController = () => {
	return [
		...
		async (req: APIReq, res: APIRes<{ recipes: IStoredRecipe[] }>) => {
			...
			const sharedRecipes = await getSharedRecipes(userId);

			res.status(200).json({
				success: true,
				message: 'Successfully fetched recipes',
				data: { recipes: [...userRecipes, ...sharedRecipes] }
			});
		}
	];
};
...
export const v0DeletePhotoController = () => {
	return async (
		...
		await removeFile(`storage/recipes/${photoId}`);
		await removeFile(`storage/recipes/${photoId}__preview.jpg`);

		const updatedRecipe = await updateRecipe(recipeId, {
			...storedRecipe,
			photos: storedRecipe.photos.filter(photo => photo !== photoId)
		});
